...
*/
func (internal getTestHandler) Execute(c *gin.Context) {
	...
	if util.IsEmptyObject(queryType) || queryType != enum.TYPE_REQUEST_HTTP_GET_TEST.String() {
		response = internal.buildResponseFailed(msgId, enum.BAD_REQUEST, "param type is not valid")
	} else if util.IsEmptyObject(name) {
		response = internal.buildResponseFailed(msgId, enum.BAD_REQUEST, "param name is empty")
	} else {
		response = internal.buildResponse(msgId, name, userId, acl)
	}

	c.JSON(200, response)
}

func (internal getTestHandler) buildResponse(msgId string, name string, userId string, acl string) handlerTest.GetTestResponse {
...
func (internal getTestHandler) buildResponseFailed(msgId string, responseCode enum.RESP_CODE, msgError string) handlerTest.GetTestResponse {
...
func (internal getTestHandler) decodeJwtToken(msgId string, token string) handlerAuth.JWTToken {
